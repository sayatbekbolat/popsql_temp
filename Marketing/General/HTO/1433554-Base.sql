...
DECLARE daysP INT64 DEFAULT 32;
...
DELETE FROM `gusa-funnel-dwh.HTO_Data.HTO_cohort_assist_traffic` WHERE TRUE;
INSERT INTO `gusa-funnel-dwh.HTO_Data.HTO_cohort_assist_traffic` (backendVisitorId, Date, Datetime, Media, Campaign_Type, campaign, adContent, keyword)
(
    SELECT DISTINCT
        scd.value AS backendVisitorId,
        DATE(TIMESTAMP_MICROS(visitStartTime*1000000), "UTC") AS Date,
        DATETIME(TIMESTAMP_MICROS(visitStartTime*1000000), "UTC") AS Datetime,
        Media,
...
DELETE FROM `gusa-funnel-dwh.HTO_Data.HTO_cohort_assist_transactions` WHERE TRUE;
INSERT INTO `gusa-funnel-dwh.HTO_Data.HTO_cohort_assist_transactions` (backendVisitorId, Datetime, transactionId, Media, Campaign_Type, campaign, adContent, keyword, isHTO)
(
    SELECT DISTINCT
        scd.value AS backendVisitorId,
        DATETIME(TIMESTAMP_MICROS(visitStartTime*1000000), "UTC") AS Datetime,
